...
import { Search, Info, ListChecks, Users2 } from "lucide-react";
...
const DirectorProjects = ({ darkMode }: DirectorProjectsProps) => {
    ...
    const activeTab = useMemo(() => {
        return location.pathname.endsWith("/assigned") ? 'assigned-to-me' : 'my-projects';
    }, [location.pathname]);

    const handleTabChange = (tab: string) => {
        if (tab === 'assigned-to-me') {
            navigate('/dashboard/director/projects/assigned', { replace: true });
        } else {
            navigate('/dashboard/director/projects', { replace: true });
        }
    };
    ...
    useEffect(() => {
        ...
        const projectsAssignedToDirector: Project[] = [
            { id: "101", name: "Digital Transformation Initiative", members: 15, progress: 20, dueDate: threeMonths.toISOString().split('T')[0], kickOffDate: today.toISOString().split('T')[0], status: 'active', priority: 'high', assignedBy: 'Vice President', supervisorStatus: 'accepted' },
            { id: "102", name: "Annual Financial Reporting Automation", members: 7, progress: 50, dueDate: nextMonth.toISOString().split('T')[0], kickOffDate: today.toISOString().split('T')[0], status: 'active', priority: 'medium', assignedBy: 'President', supervisorStatus: 'accepted' },
        ];

        setMyProjects(createdProjects);
        setAssignedProjects(projectsAssignedToDirector);
    }, []);
                    ...
                    <TabsContent value="my-projects">
                        <Card className={`mt-4 ${darkMode ? 'bg-zinc-800 border-zinc-700' : ''}`}>
                            ...
                            <CardContent className="pt-4">
                                <ProjectDataTable projects={filteredProjects} type="my-projects" />
                            </CardContent>
                        </Card>
                    </TabsContent>
